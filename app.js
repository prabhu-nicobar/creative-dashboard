...
function sc(label,val,b,fmt,extra,base,scope,bn){
 let bar='';
 if(base!=null){
  const d=val-base;
  const tone=d>=3?'up':d<=-3?'dn':'fl';
  const where=scope==='cohort'?'in this group':'account-wide, because this group has too few';
  bar=`<span class="vsb ${tone}${scope==='account'?' wide':''}" title="The typical ${fmt} scores ${base} on ${label} ${where}, measured across ${esc(D.baselineLabel)} from ${bn} creatives. This one is ${d>0?'+':''}${d} against that.">
   <i>${esc(fmt)} median</i> ${base}${scope==='account'?'*':''} <b>${d>0?'+':''}${d}</b></span>`;
 }
 return `<div class="sc ${cls(b)}${extra||''}">
...
function card(a){
 ...
 return `<article class="card ${SEL.has(a.id)?'sel':''}" data-id="${a.id}">
  ...
  ${img(a.img,a.cid)}
  <div class="scores">
   ${sc('Pre-Click',a.pre,a.preB,a.f,'',a.base?a.base.pre:null,a.base&&a.base.scope,a.base&&a.base.n)}
   ${sc('Post-Click',a.post,a.postB,a.f,'',a.base?a.base.post:null,a.base&&a.base.scope,a.base&&a.base.n)}
   ${sc('Overall',a.ov,a.ovB,a.f,' ov',a.base?a.base.ov:null,a.base&&a.base.scope,a.base&&a.base.n)}</div>
  ${a.base?`<p class="basenote">Compared against the typical <b>${esc(a.f)}</b>
   ${a.base.scope==='cohort'?'in this group':'across the account, because this group runs too few to read on its own'},
   from ${a.base.n} creatives over ${esc(D.baselineLabel)}.</p>`:''}
  ...
  <div class="meta"><span class="conf ${a.conf}" title="${esc(a.confN)}"><i class="dot"></i>${esc(a.confL)}</span>
   ...
   <a class="cta" target="_blank" rel="noopener"
    href="https://adsmanager.facebook.com/adsmanager/manage/ads?act=${D.account}&selected_ad_ids=${a.id}">Ads Manager ↗</a></div>
  ${flags}
...
function viewGroups(){
 ...
 const bl=(D.baseline&&D.baseline[CK])||{};
 const blAll=(D.baseline&&D.baseline['__all__'])||{};
 const strip=m.fstrip.map(f=>{
  const b=bl[f.f]||blAll[f.f]; const wide=!bl[f.f]&&!!blAll[f.f];
  const row=(label,now,med)=>{
   const d=(med!=null&&now)?now-med:null;
   return `<div class="fb"><span>${label}</span>
    <span class="bar2"><span style="width:${now||0}%;background:${label==='Pre'?'var(--slate)':'var(--amber)'}"></span>
     ${med!=null?`<i class="tick" style="left:${med}%" title="Median ${med}"></i>`:''}</span>
    <b>${now||'—'}</b>${med!=null?`<u class="${d>=3?'up':d<=-3?'dn':'fl'}">${d>0?'+':''}${d}</u>`:''}</div>`;
  };
  return `<div class="fs"><div class="fn">${esc(f.f)}</div>
   <div class="fm">${f.n} live · ${R(f.sp)}</div>
   ${row('Pre',f.pre,b?b.pre:null)}
   ${row('Post',f.post,b?b.post:null)}
   ${b?`<div class="fmed">Median ${b.pre}/${b.post}${wide?'*':''} · ${b.n} creatives</div>`:''}
  </div>`;}).join('');
 return ARCH+MSG+`<div class="chead"><div class="ctitle"><h3>${esc(m.cat)}</h3>
  ...
  <div class="fstriphd">Formats in this group
   <i>bars show this window · the tick and the figure below show the ${esc(D.baselineLabel)} median for that format in this group${Object.keys(bl).length<m.fstrip.length?', or account-wide where marked *':''}</i></div>
...
function viewHowto(){
 ...
 <p>Under each score you will see something like <b>SNG median 46 &nbsp;+7</b>. That is the median score for
